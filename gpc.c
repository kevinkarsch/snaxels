...
gpc_polygon gpc_make_null_poly() 
{ 
	gpc_polygon p; 
	p.contour = NULL; 
	p.hole = NULL; 
...
void gpc_fix_self_intersections(gpc_vertex_list *contour, 
							   gpc_polygon **result_polygons, 
							   int *num_result_polygons)
{
	//Find intersection points, add them to the contour
	int num_intersections = 0, i, j;
	gpc_intersection *intersections = NULL;
	double u_a, u_b, denom, x_int, y_int;
	gpc_vertex *p1, *p2, *p3, *p4;
	for(i = 0; i < contour->num_vertices; i++) {
		for(j = i+1; j < contour->num_vertices; j++) {
			if( i == 0 ) p1 = &contour->vertex[contour->num_vertices-1];
			else		 p1 = &contour->vertex[i-1];
			p2 = &contour->vertex[i];
			p3 = &contour->vertex[j-1];
			p4 = &contour->vertex[j];
			denom = (p4->y - p3->y)*(p2->x - p1->x) - (p4->x - p3->x)*(p2->y - p1->y);
			if( ! EQ(denom, 0.f) ) {
				u_a = ((p4->x - p3->x)*(p1->y - p3->y) - (p4->y - p3->y)*(p1->x - p3->x)) / denom;
				if( GPC_EPSILON < u_a && u_a < 1.f-GPC_EPSILON ) {
					u_b = ((p2->x - p1->x)*(p1->y - p3->y) - (p2->y - p1->y)*(p1->x - p3->x)) / denom;
					if( GPC_EPSILON < u_b && u_b < 1.f-GPC_EPSILON ) { //Intersection!
						x_int = p1->x + u_a * (p2->x - p1->x);
						y_int = p1->y + u_a * (p2->y - p1->y);

						//Add to intersection list
						num_intersections += 2;
						intersections = (gpc_intersection*)realloc( intersections,
							sizeof(gpc_intersection)*(num_intersections) );

						intersections[num_intersections-2].x = x_int;
						intersections[num_intersections-2].y = y_int;
						intersections[num_intersections-2].primary_idx = i;
						intersections[num_intersections-2].second_idx = j;

						intersections[num_intersections-1].x = x_int;
						intersections[num_intersections-1].y = y_int;
						intersections[num_intersections-1].primary_idx = j;
						intersections[num_intersections-1].second_idx = i;
					}
				}
			}
		}
	}
	// Decompose contour into non-intersecting polygon
// Something broken here! default to else
	if( 0 ) { // num_intersections > 0 ) {
		int start_idx, current_idx;
		int direction; // true=forward or false=backward through the list
		//Make one new contour per intersection
		*num_result_polygons = num_intersections;
		*result_polygons = (gpc_polygon*)realloc(*result_polygons, sizeof(gpc_polygon)*num_intersections);
		for(i = 0; i < num_intersections; i++) {
			//Initialize contour with current intersection vertex
			gpc_vertex_list new_contour;
			new_contour.num_vertices = 1;
			new_contour.vertex = (gpc_vertex*)malloc(sizeof(gpc_vertex));
			new_contour.vertex[0].x = intersections[i].x;
			new_contour.vertex[0].y = intersections[i].y;
			start_idx = current_idx = intersections[i].primary_idx;
			direction = 1;
			//direction = 1;
			do { //Accumulate new contour, stop when initial point is reached
				if( direction == 1 ) {
					if( ++current_idx >= contour->num_vertices ) current_idx = 0;
				} else {
					if( --current_idx < 0 ) current_idx = contour->num_vertices-1;
				}
				//Add vertex
				new_contour.vertex = (gpc_vertex*)realloc(new_contour.vertex, sizeof(gpc_vertex) *(++new_contour.num_vertices) );
				new_contour.vertex[new_contour.num_vertices-1].x = contour->vertex[current_idx].x;
				new_contour.vertex[new_contour.num_vertices-1].y = contour->vertex[current_idx].y;
				//Check if intersection exists between current_idx and current_idx+1
				for(j = 0; j < num_intersections; j++) {
					if( current_idx == intersections[j].primary_idx ) {
						//Add intersection
						new_contour.vertex = (gpc_vertex*)realloc(new_contour.vertex, 
							sizeof(gpc_vertex) *(++new_contour.num_vertices) );
						new_contour.vertex[new_contour.num_vertices-1].x = intersections[j].x;
						new_contour.vertex[new_contour.num_vertices-1].y = intersections[j].y;
						//Update current idx
						current_idx = intersections[j].second_idx;
						//Update direction
			...
			(*result_polygons)[i] = gpc_make_null_poly();
			gpc_add_contour( &((*result_polygons)[i]), &new_contour, 0 );
		}
	} else { //No points intersect (=> simple poly), return the original contour
		*result_polygons = (gpc_polygon*)realloc(*result_polygons, sizeof(gpc_polygon)*(++(*num_result_polygons)));
		(*result_polygons)[(*num_result_polygons)-1] = gpc_make_null_poly();
		gpc_add_contour( &((*result_polygons)[(*num_result_polygons)-1]), contour, 0 );
	}
}
